...
- `PostProcessing\ChainDataExtraction.py`: A minimal `GetDist` loader: it reads a raw chain file into an `MCSamples` object for quick checks of means and marginals, without the full post-processing machinery. Used for debugging and not kept aligned with later development.
...
- `PostProcessing\diff_failure_audit.py`: When `posterior_background_heatmaps.py` records a failed `CLASS` call, this script diffs the preserved `.ini` file against the configuration reconstructed from the best-fit, YAML, and sampled chain values, and reports where the two disagree. Used for debugging.
...
- `PostProcessing\omega_table.py`: Collects $\Omega$ values at $z=0$ for the hyperbolic tangent and $`\Lambda`$CDM models across different likelihood combinations and writes a LaTeX table.
- `PostProcessing\posterior_background_heatmaps.py`: Creates heatmaps of the cosmic evolution of various parameters by sampling through the MCMC chains of accepted parameter combinations and calling CLASS for this sample.
- `PostProcessing\regenerate_thesis_figures.py`: Recreates all thesis figures and tables based on newest data/MCMC chains/best fit values: it calls the plotting scripts above with the dataset-matched pairing of hyperbolic and $`\Lambda`$CDM best-fits from the MCMC folder (on [Zenodo](https://doi.org/10.5281/zenodo.21417680)) and writes a provenance manifest, so every figure can be traced back to the exact files it was built from.
- `PostProcessing\stage_thesis_figures.py`: In my thesis `LaTeX` folder, I have a `figures/` subfolder. I recreate the plots here in the same folder in order to just copy it over and keep the same relative paths, which are important within the `PGF` files.
- `PostProcessing\Swampland_Survival_Rate.sh`: Admittedly not written in `Python`, yet a helper script for initial inspection and debugging nonetheless. This bash script counts how many chain samples survive a hard de Sitter Conjecture cut. The production runs use a smooth Gaussian penalty instead, so the hard cut is a conservative cross-check.
