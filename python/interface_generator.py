...
try:
    ...
    raise ImportError(
        "You have to install the colorlog module" " with pip, or easy-install."
    )
...
NAMING_CONVENTION = {
    ...
    "thermo": {"python": "thermodynamics", "function": "thermodynamics"},
    "perturbs": {"python": "perturbations", "function": "perturb"},
    "transfers": {"python": "transfer", "function": "transfer"},
    "primordial": {"python": "primordial", "function": "primordial"},
    "spectra": {"python": "spectra", "function": "spectra"},
    "lensing": {"python": "lensing", "function": "lensing"},
    "nonlinear": {"python": "nonlinear", "function": "nonlinear"},
    "output": {"python": "output", "function": "output"},
}
        ...
        for line in header_file:
            if in_modules:
                ...
                if line.find("common") == -1 and line.find("input") == -1:
                    headers.append("../include/%s" % line.split()[-1].strip('"'))
    ...
    with open(header, "r") as header_file:
        ...
        for line in header_file:
            ...
            if not main_struct_finished and not in_struct:
                if line.find("enum ") != -1 and line.find("{") != -1:
                    ...
                    if line.find(";") == -1:
                        ...
                        output_file.write(SPACING + "cdef enum %s:\n" % enum_name)
            ...
            if line.find("struct ") != -1 and not main_struct_finished:
                ...
                structs[struct_name] = {}
                structs[struct_name].update(NAMING_CONVENTION[struct_name])
                ...
                if line.find("};\n") != -1:
                    ...
                    if line.strip() == "" or line.strip()[:2] == "/*":
                        continue
                    logger.debug("potentially non empty line: %s" % line.strip())
                    ...
                    if line.find(";") == -1 and not comment_partially_recovered:
                        ...
                        if var_part.find("*") != -1:
                            ...
                            if var_part.find(",") == -1:
                                ...
                                structs[struct_name][var_name] = [var_type, var_stars]
                            else:
                                # Count how many variables are defined
                                multiple_var = True
                                all_vars = [
                                    elem.strip()
                                    for elem in var_part.split("*")[-1].split(",")
                                ]
                                var_type, var_stars = var_part.strip().split()[:2]
                                for var in all_vars:
                                    structs[struct_name][var] = [var_type, var_stars]
                        else:
                            ...
                            if var_part.find(",") == -1:
                                ...
                                all_vars = [
                                    elem.strip()
                                    for elem in var_part.split()[2:].split(",")
                                ]
                        ...
                        if not multiple_var:
                            output_file.write(
                                ...
                                + " ".join(
                                    [
                                        elem
                                        for elem in [var_type, var_stars, var_name]
                                        if elem
                                    ]
                                )
                                + "\n"
                            ...
                            for var in all_vars:
                                output_file.write(
                                    ...
                                    + " ".join(
                                        [
                                            elem
                                            for elem in [var_type, var_stars, var]
                                            if elem
                                        ]
                                    )
                                    + "\n"
                ...
                if not in_function_definitions:
                    ...
                    if line.find("(") != -1:
                        ...
                        output_file.write(SPACING + "%s %s(" % (func_type, func_name))
                    elif in_function:
...
def create_wrapper_class(struct_name, struct, of, logger):
    ...
    _, argument_names = struct["init"][0], struct["init"][1:]
