                              ...
                              ErrorMsg error_message)
{
  ...
  */
  ...
  {
    class_run_parallel_mutable(=, class_alloc(PhiL, (lmax + 2) * sizeof(double), error_message); if ((K == 1) && ((int)(beta + 0.2) == (lmax + 1))) {
      PhiL[lmax+1] = 0.0;
      lmax--; }
                               // Use backwards method:
                               hyperspherical_backwards_recurrence(K, MIN(l_recurrence_max, lmax) + 1, beta, pHIS->x[j], pHIS->sinK[j], pHIS->cotK[j], sqrtK, one_over_sqrtK, PhiL);
      ...
      pHIS->dphi[k*nx+j] = l*pHIS->cotK[j]*PhiL[l]-sqrtK[l+1]*PhiL[l+1]; } free(PhiL); return _SUCCESS_;);
  }
  ...
  for (j = xfwdidx; j < nx; j += _HYPER_CHUNK_)
  {
    class_run_parallel_mutable(=,
                               ...
                               class_alloc(PhiL, (lmax + 2) * sizeof(double) * current_chunk, error_message); if ((K == 1) && ((int)(beta + 0.2) == (lmax + 1))) {
      for(k=0; k<(lmax+2)*current_chunk; k++){
        PhiL[k] = 0.0;
      }
      lmax--; } hyperspherical_forwards_recurrence_chunk(K, MIN(l_recurrence_max, lmax) + 1, beta, pHIS->x + j, pHIS->sinK + j, pHIS->cotK + j, current_chunk, sqrtK, one_over_sqrtK, PhiL);
