...
static int evolver_rk_inner(int (*derivs)(double x,
										  double *y,
										  double *dy,
			   ...
			   ErrorMsg error_message)
{
	...
	dy = (double *)malloc(y_size * sizeof(double));
	if (dy == NULL)
	{
		cleanup_generic_integrator(&gi);
		class_alloc_message(error_message, "dy", y_size * (int)sizeof(double));
		return _FAILURE_;
	}

	status = evolver_rk_inner(
		derivs, x_ini, x_end, y, used_in_output, y_size,
		parameters_and_workspace_for_derivs, tolerance, minimum_variation,
		evaluate_timescale, timestep_over_timescale,
		x_sampling, x_size, output, print_variables,
		error_message,
		&gi, dy);
