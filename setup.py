...
GCCPATH_STRING = sbp.Popen(
    ["gcc", "-print-libgcc-file-name"], stdout=sbp.PIPE
).communicate()[0]
...
with open(os.path.join(include_folder, "common.h"), "r") as v_file:
    for line in v_file:
        if line.find("_VERSION_") != -1:
            ...
            VERSION = (
                line.split()[-1][2:-1] + ".0"
            )  # Always set a sub-version number here for subsequent uploads (!)
            break
...
def package_files(directory):
    ...
    wanted_paths = {
        ...
        for d in ["tools", "source", "main", "python", "include"]
    }
    for path, directories, filenames in os.walk(directory):
        # Only include those directories that we actually want
        if path in wanted_paths or (
            path.startswith(os.path.join(directory, "external"))
            and not "RealSpaceInterface" in path
        ):
...
class classy_builder(build_ext):
    ...
    def build_extension(self, ext):
        ...
        if returncode != 0:
            raise RuntimeError(
                "Unknown error occurred -- the Makefile compilation of class failed (return code %i). Run the installation with '-v' and check why this is the case from the makefile command output"
                % returncode
            )
