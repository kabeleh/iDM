    ...
    double *pvecthermo)
{
  ...
  if (z >= pth->z_table[pth->tt_size - 1])
  {
    ...
    if (pba->has_idm == _TRUE_)
    {
      ...
      if (pth->has_idm_b == _TRUE_)
      {
        ...
        pvecthermo[pth->index_th_R_idm_b] = (pvecback[pba->index_bg_a] * pvecback[pba->index_bg_rho_b] * pth->cross_idm_b * pth->n_coeff_idm_b / (m_b + pth->m_idm)) * pow(T_diff_idm_b, (pth->n_index_idm_b + 1.0) / 2.0) * FHe * (3.e-4 * pow(_c_, 4.) / (8. * _PI_ * _Mpc_over_m_ * _G_ * _eV_)); /* conversion coefficient for the units */
        pvecthermo[pth->index_th_dR_idm_b] = pvecthermo[pth->index_th_R_idm_b] * pvecback[pba->index_bg_a] * pvecback[pba->index_bg_H] * (-2. - (1. + z) * (pth->n_index_idm_b + 1.0) / 2.0 * (pba->T_cmb * _k_B_ / _eV_ / m_b + pba->T_cmb * _k_B_ / _eV_ / pth->m_idm) / T_diff_idm_b);
      }
      ...
      if (pth->has_idm_dr == _TRUE_)
      {
        ...
        pvecthermo[pth->index_th_dddmu_idm_dr] = (pvecback[pba->index_bg_H] * pvecback[pba->index_bg_H] / (1. + z) * (pth->n_index_idm_dr - 1.) - pvecback[pba->index_bg_H_prime]) * pth->n_index_idm_dr / (1. + z) * pvecthermo[pth->index_th_dmu_idm_dr];
  ...
  else
  {
    ...
    if (((pth->reio_parametrization == reio_half_tanh) && (z < 2 * pth->z_reio)) || ((pth->reio_parametrization == reio_inter) && (z < 50.)))
    {
    ...
    struct thermodynamics *pth)
{
  ...
  Neff_bbn = (pvecback[pba->index_bg_Omega_r] * pvecback[pba->index_bg_rho_crit] - pvecback[pba->index_bg_rho_g]) / (7. / 8. * pow(4. / 11., 4. / 3.) * pvecback[pba->index_bg_rho_g]);
    ...
    struct thermo_reionization_parameters *preio)
{
  ...
  case reio_bins_tanh:
    ...
    preio->reionization_parameters[preio->index_re_first_z + preio->re_z_size - 1] =
        preio->reionization_parameters[preio->index_re_first_z + preio->re_z_size - 2] + 2. * (preio->reionization_parameters[preio->index_re_first_z + preio->re_z_size - 2] - preio->reionization_parameters[preio->index_re_first_z + preio->re_z_size - 3]);
    ...
    preio->reionization_parameters[preio->index_re_first_z] =
        2. * preio->reionization_parameters[preio->index_re_first_z + 1] - preio->reionization_parameters[preio->index_re_first_z + 2];
  ...
  case reio_many_tanh:
    ...
    preio->reionization_parameters[preio->index_re_first_z + preio->re_z_size - 1] =
        preio->reionization_parameters[preio->index_re_first_z + preio->re_z_size - 2] + ppr->reionization_start_factor * pth->many_tanh_width;
    ...
    preio->reionization_parameters[preio->index_re_first_z] =
        preio->reionization_parameters[preio->index_re_first_z + 1] - ppr->reionization_start_factor * pth->many_tanh_width;
  ...
  for (index_interval = 0; index_interval < interval_number; index_interval++)
  {
    ...
    if ((pth->reio_z_or_tau == reio_tau) && (index_interval == ptw->ptdw->index_ap_reio))
    {
      ...
      class_call_except(thermodynamics_reionization_evolve_with_tau(&tpaw,
                        ...
                        free(interval_limit); free(mz_output));
    }
  ...
  */
  ...
  else
  {
    ...
    if (pth->has_idm_b == _TRUE_)
    {
      ...
      dy[ptv->index_ti_D_Tmat] += 2. * pvecback[pba->index_bg_rho_idm] / pvecback[pba->index_bg_rho_b] * mu_bar / (pth->m_idm * _eV_ / _c_ / _c_ + _m_p_) * ptdw->R_idm_b * (Tmat - ptdw->T_idm) / (pvecback[pba->index_bg_a] * pvecback[pba->index_bg_H] * (1. + z));
    }
  }
    ...
    ErrorMsg error_message)
{
  ...
  /* wb = (k_B/mu) Tb */
  pth->thermodynamics_table[(pth->tt_size - index_z - 1) * pth->th_size + pth->index_th_wb] = _k_B_ / (_c_ * _c_ * _m_H_) * (1. + (1. / _not4_ - 1.) * ptw->YHe + x * (1. - ptw->YHe)) * Tmat;

  /* cb2 = (k_B/mu) Tb (1-1/3 dlnTb/dlna) = (k_B/mu) Tb (1 + 1/3 (1+z) dlnTb/dz) */
  pth->thermodynamics_table[(pth->tt_size - index_z - 1) * pth->th_size + pth->index_th_cb2] = _k_B_ / (_c_ * _c_ * _m_H_) * (1. + (1. / _not4_ - 1.) * ptw->YHe + x * (1. - ptw->YHe)) * Tmat * (1. + (1. + z) * dTmat / Tmat / 3.);

  /* dkappa/dtau = a n_e x_e sigma_T = a^{-2} n_e(today) x_e sigma_T (in units of 1/Mpc) */
  pth->thermodynamics_table[(pth->tt_size - index_z - 1) * pth->th_size + pth->index_th_dkappa] = (1. + z) * (1. + z) * ptw->SIunit_nH0 * x * sigmaTrescale * _sigma_ * _Mpc_over_m_;
    ...
    double *pvecback)
{
  ...
  for (index_tau = 0; index_tau < pth->tt_size; index_tau++)
  {
    ...
    pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_ddkappa] =
        1. / 6. / pth->thermodynamics_table[(pth->tt_size - 1 - index_tau) * pth->th_size + pth->index_th_dkappa] * (R * R / (1 + R) + 16. / 15.) / (1. + R);
  }
  ...
  for (index_tau = 0; index_tau < pth->tt_size; index_tau++)
  {
    ...
    pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_r_d] =
        2. * _PI_ * sqrt(16. / (15. * 6. * 3.) * tau_ini / dkappa_ini + pth->thermodynamics_table[(pth->tt_size - 1 - index_tau) * pth->th_size + pth->index_th_g]);
  }
                                                    ...
                                                    double *pvecback)
{
  ...
  if (pth->has_idm_dr == _TRUE_)
  {
    ...
    for (index_tau = pth->tt_size - 1; index_tau >= 0; index_tau--)
    {
      ...
      pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_g_idm_dr] =
          pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_ddmu_idm_dr] * exp(-pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_tau_idm_dr]);
    }
  ...
  if (pth->has_idm_dr == _TRUE_)
  {
    ...
    if (pth->n_index_idm_dr >= 2)
    {
      ...
      while ((1. / pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_dmu_idm_dr] / pth->tau_table[index_tau] < ppr->idr_streaming_trigger_tau_c_over_tau) && index_tau > 0)
        index_tau--;
    }
    else
    {
      index_tau = 0;
      while ((1. / pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_dmu_idm_dr] / pth->tau_table[index_tau] < ppr->idr_streaming_trigger_tau_c_over_tau) && index_tau < pth->tt_size - 1)
        index_tau++;
    }
    ...
    double *pvecback)
{
  ...
  if (pth->compute_damping_scale == _TRUE_)
  {
    ...
    pth->rd_rec = (pth->z_table[index_tau + 1] - pth->z_rec) / (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) * pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_r_d] + (pth->z_rec - pth->z_table[index_tau]) / (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) * pth->thermodynamics_table[(index_tau + 1) * pth->th_size + pth->index_th_r_d];
  }

  /** - find time (always after recombination) at which tau_c/tau falls below some threshold, defining tau_free_streaming */
  while (1. / pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_dkappa] / pth->tau_table[index_tau] < ppr->radiation_streaming_trigger_tau_c_over_tau)
  {
  ...
  pth->z_star = pth->z_table[index_tau - 1] +
                (1. / _E_ - pth->thermodynamics_table[(index_tau - 1) * pth->th_size + pth->index_th_exp_m_kappa]) / (pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_exp_m_kappa] - pth->thermodynamics_table[(index_tau - 1) * pth->th_size + pth->index_th_exp_m_kappa]) * (pth->z_table[index_tau] - pth->z_table[index_tau - 1]);
  ...
  if (pth->compute_damping_scale == _TRUE_)
  {
    ...
    pth->rd_star = (pth->z_table[index_tau + 1] - pth->z_star) / (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) * pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_r_d] + (pth->z_star - pth->z_table[index_tau]) / (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) * pth->thermodynamics_table[(index_tau + 1) * pth->th_size + pth->index_th_r_d];
  }
    ...
    double *pvecback)
{
  ...
  pth->z_d = pth->z_table[index_tau - 1] +
             (1. - pth->thermodynamics_table[(index_tau - 1) * pth->th_size + pth->index_th_kappa_b]) / (pth->thermodynamics_table[(index_tau)*pth->th_size + pth->index_th_kappa_b] - pth->thermodynamics_table[(index_tau - 1) * pth->th_size + pth->index_th_kappa_b]) * (pth->z_table[index_tau] - pth->z_table[index_tau - 1]);
    ...
    double *x)
{
  ...
  case reio_camb:
    ...
    else
    {
      ...
      argument = (pow((1. + preio->reionization_parameters[preio->index_re_reio_redshift]),
                 ...
                 (preio->reionization_parameters[preio->index_re_reio_exponent] * pow((1. + preio->reionization_parameters[preio->index_re_reio_redshift]),
                                                                                      (preio->reionization_parameters[preio->index_re_reio_exponent] - 1.))) /
                 preio->reionization_parameters[preio->index_re_reio_width];

      *x = (preio->reionization_parameters[preio->index_re_xe_after] - preio->reionization_parameters[preio->index_re_xe_before]) * (tanh(argument) + 1.) / 2. + preio->reionization_parameters[preio->index_re_xe_before];
  ...
  case reio_half_tanh:
    ...
    else
    {
      ...
      *x = (preio->reionization_parameters[preio->index_re_xe_after] - preio->reionization_parameters[preio->index_re_xe_before]) * tanh(argument) + preio->reionization_parameters[preio->index_re_xe_before];
    }
    break;
  ...
  case reio_bins_tanh:
    ...
    {
      i = 0;
      while (preio->reionization_parameters[preio->index_re_first_z + i + 1] < z)
        i++;
      ...
      */
      ...
      {
        z_jump = preio->reionization_parameters[preio->index_re_first_z + i] + 0.5 * (preio->reionization_parameters[preio->index_re_first_z + i] - preio->reionization_parameters[preio->index_re_first_z + i - 1]);
      }
      else
      {
        z_jump = 0.5 * (preio->reionization_parameters[preio->index_re_first_z + i + 1] + preio->reionization_parameters[preio->index_re_first_z + i]);
      }

      /* implementation of the tanh jump */
      *x = preio->reionization_parameters[preio->index_re_first_xe + i] + 0.5 * (tanh((z - z_jump) / preio->reionization_parameters[preio->index_re_step_sharpness]) + 1.) * (preio->reionization_parameters[preio->index_re_first_xe + i + 1] - preio->reionization_parameters[preio->index_re_first_xe + i]);
    }
    break;
    ...
    else if (z > preio->reionization_parameters[preio->index_re_first_z])
    {
      ...
      *x = preio->reionization_parameters[preio->index_re_xe_before];
      ...
      for (jump = 1; jump < preio->re_z_size - 1; jump++)
      {
        ...
        before = preio->reionization_parameters[preio->index_re_first_xe + preio->re_z_size - 1 - jump] - preio->reionization_parameters[preio->index_re_first_xe + preio->re_z_size - jump];
        after = 0.;
  ...
  case reio_inter:
    ...
    else
    {
      ...
      argument = (z - preio->reionization_parameters[preio->index_re_first_z + i]) / (preio->reionization_parameters[preio->index_re_first_z + i + 1] - preio->reionization_parameters[preio->index_re_first_z + i]);

      *x = preio->reionization_parameters[preio->index_re_first_xe + i] + argument * (preio->reionization_parameters[preio->index_re_first_xe + i + 1] - preio->reionization_parameters[preio->index_re_first_xe + i]);
                                  ...
                                  double *pvecback)
{
  ...
  if (pba->has_idm == _TRUE_)
  {
    ...
    if (pth->has_idm_b == _TRUE_)
    {
      ...
      ptdw->R_idm_b = (pvecback[pba->index_bg_a] * pvecback[pba->index_bg_rho_b] * pth->cross_idm_b * pth->n_coeff_idm_b / (m_b + pth->m_idm)) * pow(T_diff_idm_b, (pth->n_index_idm_b + 1.0) / 2.0) * FHe * (3.e-4 * pow(_c_, 4.) / (8. * _PI_ * _Mpc_over_m_ * _G_ * _eV_));
    ...
    struct thermo_workspace *ptw)
{
  ...
  if (pba->has_idm == _TRUE_)
  {
    ...
    if ((pth->has_idm_b == _TRUE_) && (pth->n_index_idm_b > -3))
    {
      ...
      z_idm_dec = 4e4 * pow(9.15 / pth->n_coeff_idm_b * pow((1. + pba->Neff * f1nu) / (1. + 3.044 * f1nu), 1. / 2.) * (0.0224 / pba->Omega0_b / pba->h / pba->h) * pow(R_dm, (pth->n_index_idm_b + 1.) / 2.) / pow(1. + R_dm, (pth->n_index_idm_b - 1.) / 2.) * pow(10., 4 * pth->n_index_idm_b - 25) / pth->cross_idm_b, 2. / (pth->n_index_idm_b + 3));
    ...
    if ((pth->has_idm_dr == _TRUE_) && (pth->n_index_idm_dr > 0))
    {
      z_idm_dec = pow(10., 7. - 6. / pth->n_index_idm_dr) * pow(1.6 * (1e-6 / pba->Omega0_idr / pba->h / pba->h) * pow((1. + pba->Neff * f1nu) / (1. + 3.044 * f1nu), 1. / 2.) * (1e6 / pth->a_idm_dr), 1. / pth->n_index_idm_dr);
      if (pth->thermodynamics_verbose > 3)
