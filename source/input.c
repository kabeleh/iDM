                                  ...
                                  ErrorMsg errmsg)
{
  ...
  class_call(parser_read_string(pfc,
             ...
             errmsg,
             errmsg);
  /* Choose the CDM Model */
  if (flag1 == _TRUE_)
  {
    if (string_begins_with(string1, 'h') || string_begins_with(string1, 'H'))
    {
      pba->model_cdm = 1;
    }
      ...
      if (flag1 == _TRUE_)
      {
        ...
        if (entries_read < (ppr->l_max_idr - 1))
        {
          ...
          for (n = entries_read; n < (ppr->l_max_idr - 1); n++)
            ppt->alpha_idm_dr[n] = ppt->alpha_idm_dr[entries_read - 1];
        }
      }
      else
      {
  ...
  else if (ppm->primordial_spec_type == two_scales)
  {
    ...
    if (ppt->has_scalars == _TRUE_)
    {
      ...
      if ((ppt->has_bi == _TRUE_) || (ppt->has_cdi == _TRUE_) || (ppt->has_nid == _TRUE_) || (ppt->has_niv == _TRUE_))
      {
        ...
        { // general case, but if pii2 or pri2=0, the code interprets it as a request for n_iso=n_ad or n_cor=0 respectively
          if (pii2 == 0.)
          {
            n_iso = ppm->n_s;
          }
