  ...
  if (z == 0.)
  {
    ...
    for (index_k = 0; index_k < ppt->k_size[index_md]; index_k++)
    {
      for (index_tp = 0; index_tp < ppt->tp_size[index_md]; index_tp++)
      {
        ...
        {
          tkfull[(index_k * ppt->ic_size[index_md] + index_ic) * ppt->tp_size[index_md] + index_tp] = ppt->sources[index_md][index_ic * ppt->tp_size[index_md] + index_tp][(ppt->tau_size - 1) * ppt->k_size[index_md] + index_k];
        }
      }
    }
  }
    ...
    double *data)
{
  ...
  for (index_k = 0; index_k < ppt->k_size[index_md]; index_k++)
  {
    for (index_tp = 0; index_tp < ppt->tp_size[index_md]; index_tp++)
    {
      for (index_ic = 0; index_ic < ppt->ic_size[index_md]; index_ic++)
      {
        tkfull[(index_k * ppt->ic_size[index_md] + index_ic) * ppt->tp_size[index_md] + index_tp] = ppt->late_sources[index_md][index_ic * ppt->tp_size[index_md] + index_tp][index_tau * ppt->k_size[index_md] + index_k];
      }
    }
  }
      ...
      for (index_k = ppt->k_size[index_md] - 1; index_k >= 0; index_k--)
      {
        ...
        class_run_parallel(with_arguments(ppr, pba, pth, ppt, index_md, index_ic, index_k),
            ...
            printf("\n"); }
                           ...
                           class_call(perturbations_workspace_init(ppr, pba, pth, ppt, index_md, &pw), ppt->error_message, ppt->error_message);

                           class_call(perturbations_solve(ppr, pba, pth, ppt, index_md, index_ic, index_k, &pw), ppt->error_message, ppt->error_message);

                           class_call(perturbations_workspace_free(ppt, index_md, &pw), ppt->error_message, ppt->error_message); return _SUCCESS_;
  ...
  */
  ...
  while (tau < pba->conformal_age)
  {
    ...
    if (ppt->has_cmb == _TRUE_)
    {
      ...
      a_primeprime_over_a = pvecback[pba->index_bg_H_prime] * pvecback[pba->index_bg_a] + 2. * a_prime_over_a * a_prime_over_a;
    ...
    struct perturbations *ppt)
{
  ...
  if (ppt->has_scalars == _TRUE_)
  {
    ...
    if (ppt->has_cls == _TRUE_)
    {
      ...
      k_max_cmb[ppt->index_md_scalars] = ppr->k_max_tau0_over_l_max * ppt->l_scalar_max / pba->conformal_age / pth->angular_rescaling;
    ...
    while (k < k_max)
    {
      if ((pth->has_idm_dr == _TRUE_) && (pth->n_index_idm_dr >= 2))
      {
        k *= pow(10., 1. / (ppr->k_per_decade_for_pk * ppr->idmdr_boost_k_per_decade_for_pk * pth->n_index_idm_dr + (ppr->k_per_decade_for_bao - ppr->k_per_decade_for_pk * ppr->idmdr_boost_k_per_decade_for_pk * pth->n_index_idm_dr) * (1. - tanh(pow((log(k) - log(ppr->k_bao_center * k_rec)) / log(ppr->k_bao_width), 4)))));
      }
      else
      {
        k *= pow(10., 1. / (ppr->k_per_decade_for_pk + (ppr->k_per_decade_for_bao - ppr->k_per_decade_for_pk) * (1. - tanh(pow((log(k) - log(ppr->k_bao_center * k_rec)) / log(ppr->k_bao_width), 4)))));
  ...
  if (ppt->has_vectors == _TRUE_)
  {
    ...
    if (ppt->has_cls == _TRUE_)
    {
      ...
      k_max_cmb[ppt->index_md_vectors] = ppr->k_max_tau0_over_l_max * ppt->l_vector_max / pba->conformal_age / pth->angular_rescaling;
    ...
    class_alloc(ppt->k[ppt->index_md_vectors],
                ((int)((k_max_cmb[ppt->index_md_vectors] - k_min) / k_rec / MIN(ppr->k_step_super, ppr->k_step_sub)) + 1) * sizeof(double), ppt->error_message);
  ...
  if (ppt->has_tensors == _TRUE_)
  {
    ...
    if (ppt->has_cls == _TRUE_)
    {
      ...
      k_max_cmb[ppt->index_md_tensors] = ppr->k_max_tau0_over_l_max * ppt->l_tensor_max / pba->conformal_age / pth->angular_rescaling;
    ...
    class_alloc(ppt->k[ppt->index_md_tensors],
                ((int)((k_max_cmb[ppt->index_md_tensors] - k_min) / k_rec / MIN(ppr->k_step_super, ppr->k_step_sub)) + 1) * sizeof(double), ppt->error_message);
    ...
    struct perturbations_workspace *ppw)
{
  ...
  class_test(ppw->pvecback[pba->index_bg_a] *
             ...
             ppt->error_message, "your choice of initial time for integrating wavenumbers is inappropriate: it corresponds to a time before that at which the background has been integrated. You should increase 'start_small_k_at_tau_c_over_tau_h' up to at least %g, or decrease 'a_ini_over_a_today_default'\n",
    ...
    int *pa_old)
{
  ...
  if (_vectors_)
  {
    ...
    { /* if radiation streaming approximation is off */
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off)
      { /* if tight-coupling approximation is off */
  ...
  if (_scalars_)
  {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad))
    {
      ...
      /* photon density */
      ppw->pv->y[ppw->pv->index_pt_delta_g] = -ktau_two / 3. * (1. - om * tau / 5.) * ppr->curvature_ini * s2_squared;

      /* photon velocity */
      ppw->pv->y[ppw->pv->index_pt_theta_g] = -k * ktau_three / 36. * (1. - 3. * (1. + 5. * fracb - fracnu) / 20. / (1. - fracnu) * om * tau) * ppr->curvature_ini * s2_squared;
  ...
  if (_scalars_)
  {
    ...
    if (ppt->gauge == synchronous)
    {
      ...
      ppw->pvecmetric[ppw->index_mt_h_prime_prime] =
          -2. * a_prime_over_a * ppw->pvecmetric[ppw->index_mt_h_prime] + 2. * k2 * s2_squared * y[ppw->pv->index_pt_eta] - 9. * a2 * ppw->delta_p;
      ...
      ppw->pvecmetric[ppw->index_mt_alpha_prime] = // TBC
          -2. * a_prime_over_a * ppw->pvecmetric[ppw->index_mt_alpha] + y[ppw->pv->index_pt_eta] - 4.5 * (a2 / k2) * ppw->rho_plus_p_shear;
    }
    ...
    struct perturbations_workspace *ppw)
{
  ...
  if (_scalars_)
  {
    ...
    ppw->delta_rho = ppw->pvecback[pba->index_bg_rho_g] * delta_g + ppw->pvecback[pba->index_bg_rho_b] * y[ppw->pv->index_pt_delta_b];                  // contribution to total perturbed stress-energy
    ppw->rho_plus_p_theta = 4. / 3. * ppw->pvecback[pba->index_bg_rho_g] * theta_g + ppw->pvecback[pba->index_bg_rho_b] * y[ppw->pv->index_pt_theta_b]; // contribution to total perturbed stress-energy
    ppw->rho_plus_p_shear = 4. / 3. * ppw->pvecback[pba->index_bg_rho_g] * shear_g;                                                                     // contribution to total perturbed stress-energy
    ppw->delta_p = 1. / 3. * ppw->pvecback[pba->index_bg_rho_g] * delta_g + ppw->pvecback[pba->index_bg_rho_b] * delta_p_b_over_rho_b;                  // contribution to total perturbed stress-energy
    ...
    if (pba->has_ncdm == _TRUE_)
    {
      ...
      if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_))
      {
        for (n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++)
        {
          rho_plus_p_theta_m += (ppw->pvecback[pba->index_bg_rho_ncdm1 + n_ncdm] + ppw->pvecback[pba->index_bg_p_ncdm1 + n_ncdm]) * ppw->theta_ncdm[n_ncdm]; // contribution to [(rho+p)theta]_matter
  ...
  if (_vectors_)
  {
    ...
    { /* if radiation streaming approximation is off */
      ...
      { /* if tight-coupling approximation is off */
        ...
        ppw->vector_source_v += 4. / 3. * a2 * ppw->pvecback[pba->index_bg_rho_g] * (-1. / 4. * _SQRT2_) * (y[ppw->pv->index_pt_delta_g] + 2. * y[ppw->pv->index_pt_delta_g] + y[ppw->pv->index_pt_shear_g]);

        ppw->vector_source_pi += 1. / 3. * a2 * ppw->pvecback[pba->index_bg_rho_g] * (6. * _SQRT2_ / 5. / sqrt(1. - 2. * pba->K / k / k)) * (4. / 3. / k * y[ppw->pv->index_pt_theta_g] + y[ppw->pv->index_pt_l3_g]);
      }
    }
    ...
    ErrorMsg error_message)
{
  ...
  if (_scalars_)
  {
    ...
    if (ppt->has_source_t == _TRUE_)
    {
      ...
      */
      ...
      if (ppt->gauge == newtonian)
      {
        ...
        {
          _set_source_(ppt->index_tp_t0) =
              ppt->switch_sw * g * (delta_g / 4. + pvecmetric[ppw->index_mt_psi]) + switch_isw * (g * (y[ppw->pv->index_pt_phi] - pvecmetric[ppw->index_mt_psi]) + exp_m_kappa * 2. * pvecmetric[ppw->index_mt_phi_prime]) + ppt->switch_dop / k / k * (g * theta_b_prime + g_prime * theta_b);
      ...
      if (ppt->gauge == synchronous)
      {
        ...
        if (pth->has_idm_g == _TRUE_)
        {
          ...
          _set_source_(ppt->index_tp_t1) =
              switch_isw * exp_m_kappa * exp_mu_idm_g * k * (pvecmetric[ppw->index_mt_alpha_prime] + 2. * a_prime_over_a * pvecmetric[ppw->index_mt_alpha] - y[ppw->pv->index_pt_eta]);
        }
        else
        {
          _set_source_(ppt->index_tp_t0) =
              ppt->switch_sw * g * (delta_g / 4. + pvecmetric[ppw->index_mt_alpha_prime]) + switch_isw * (g * (y[ppw->pv->index_pt_eta] - pvecmetric[ppw->index_mt_alpha_prime] - 2 * a_prime_over_a * pvecmetric[ppw->index_mt_alpha]) + exp_m_kappa * 2. * (pvecmetric[ppw->index_mt_eta_prime] - a_prime_over_a_prime * pvecmetric[ppw->index_mt_alpha] - a_prime_over_a * pvecmetric[ppw->index_mt_alpha_prime])) + ppt->switch_dop / k / k * (g * theta_b_prime + g_prime * theta_b);

          _set_source_(ppt->index_tp_t1) =
              switch_isw * exp_m_kappa * k * (pvecmetric[ppw->index_mt_alpha_prime] + 2. * a_prime_over_a * pvecmetric[ppw->index_mt_alpha] - y[ppw->pv->index_pt_eta]);
        }
    ...
    if (ppt->has_source_phi_prime == _TRUE_)
    {
      ...
      if (ppt->gauge == synchronous)
        _set_source_(ppt->index_tp_phi_prime) = dy[ppw->pv->index_pt_eta] - a_prime_over_a_prime * pvecmetric[ppw->index_mt_alpha] - a_prime_over_a * pvecmetric[ppw->index_mt_alpha_prime];
    }
    ...
    if (ppt->has_source_delta_tot == _TRUE_)
    {
      ...
      _set_source_(ppt->index_tp_delta_tot) = ppw->delta_rho / rho_tot + 3 * a_prime_over_a * (1 + pvecback[pba->index_bg_p_tot] / pvecback[pba->index_bg_rho_tot]) * theta_over_k2;
    }

    /* delta_g */
    if (ppt->has_source_delta_g == _TRUE_)
    {
      _set_source_(ppt->index_tp_delta_g) = delta_g + 4. * a_prime_over_a * theta_over_k2; // N-body gauge correction
    }

    /* delta_baryon */
    if (ppt->has_source_delta_b == _TRUE_)
    {
      _set_source_(ppt->index_tp_delta_b) = y[ppw->pv->index_pt_delta_b] + 3. * a_prime_over_a * theta_over_k2; // N-body gauge correction
    }
    ...
    {
      _set_source_(ppt->index_tp_delta_idm) = y[ppw->pv->index_pt_delta_idm] + 3. * a_prime_over_a * theta_over_k2; // N-body gauge correction
    }

    /* delta_dcdm */
    if (ppt->has_source_delta_dcdm == _TRUE_)
    {
      _set_source_(ppt->index_tp_delta_dcdm) = y[ppw->pv->index_pt_delta_dcdm] + (3. * a_prime_over_a + a * pba->Gamma_dcdm) * theta_over_k2; // N-body gauge correction;
    }

    /* delta_fld */
    if (ppt->has_source_delta_fld == _TRUE_)
    {
      _set_source_(ppt->index_tp_delta_fld) = ppw->delta_rho_fld / pvecback[pba->index_bg_rho_fld] + 3. * a_prime_over_a * (1. + pvecback[pba->index_bg_w_fld]) * theta_over_k2; // N-body gauge correction
    }
    ...
    if (ppt->has_source_delta_dr == _TRUE_)
    {
      ...
      _set_source_(ppt->index_tp_delta_dr) = y[ppw->pv->index_pt_F0_dr] / f_dr + 4. * a_prime_over_a * theta_over_k2; // N-body gauge correction
    }
    ...
    {
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
        _set_source_(ppt->index_tp_delta_ur) = y[ppw->pv->index_pt_delta_ur] + 4. * a_prime_over_a * theta_over_k2; // N-body gauge correction
      else
        _set_source_(ppt->index_tp_delta_ur) = ppw->rsa_delta_ur + 4. * a_prime_over_a * theta_over_k2; // N-body gauge correction
    }
    ...
    {
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off)
        _set_source_(ppt->index_tp_delta_idr) = y[ppw->pv->index_pt_delta_idr] + 4. * a_prime_over_a * theta_over_k2; // N-body gauge correction
      else
        _set_source_(ppt->index_tp_delta_idr) = ppw->rsa_delta_idr + 4. * a_prime_over_a * theta_over_k2; // N-body gauge correction
    }
    ...
    if (ppt->has_source_delta_ncdm == _TRUE_)
    {
      ...
      {
        _set_source_(index_tp) = ppw->delta_ncdm[index_tp - ppt->index_tp_delta_ncdm1] + 3. * a_prime_over_a * (1 + pvecback[index_tp - ppt->index_tp_delta_ncdm1 + pba->index_bg_p_ncdm1] / pvecback[index_tp - ppt->index_tp_delta_ncdm1 + pba->index_bg_rho_ncdm1]) * theta_over_k2; // N-body gauge correction
      }
    }
    ...
    {
      _set_source_(ppt->index_tp_theta_tot) = ppw->rho_plus_p_theta / (pvecback[pba->index_bg_rho_tot] + pvecback[pba->index_bg_p_tot]) + theta_shift; // N-body gauge correction
    }
    ...
    if (ppt->has_source_theta_tot == _TRUE_)
    {
      _set_source_(ppt->index_tp_theta_tot) = ppw->rho_plus_p_theta / ppw->rho_plus_p_tot + theta_shift; // N-body gauge correction
    }
    ...
    {
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
        _set_source_(ppt->index_tp_theta_g) = y[ppw->pv->index_pt_theta_g] + theta_shift; // N-body gauge correction
      else
        _set_source_(ppt->index_tp_theta_g) = ppw->rsa_theta_g + theta_shift; // N-body gauge correction
    }

    /* theta_baryon */
    if (ppt->has_source_theta_b == _TRUE_)
    {
      _set_source_(ppt->index_tp_theta_b) = y[ppw->pv->index_pt_theta_b] + theta_shift; // N-body gauge correction
    }

    /* theta_cdm */
    if (ppt->has_source_theta_cdm == _TRUE_)
    {
      _set_source_(ppt->index_tp_theta_cdm) = y[ppw->pv->index_pt_theta_cdm] + theta_shift; // N-body gauge correction
    }

    /* theta_idm */
    if (ppt->has_source_theta_idm == _TRUE_)
    {
      _set_source_(ppt->index_tp_theta_idm) = y[ppw->pv->index_pt_theta_idm] + theta_shift; // N-body gauge correction
    }

    /* theta_dcdm */
    if (ppt->has_source_theta_dcdm == _TRUE_)
    {
      _set_source_(ppt->index_tp_theta_dcdm) = y[ppw->pv->index_pt_theta_dcdm] + theta_shift; // N-body gauge correction
    }
    ...
    if (ppt->has_source_theta_fld == _TRUE_)
    {
      ...
      _set_source_(ppt->index_tp_theta_fld) = ppw->rho_plus_p_theta_fld / (1. + w_fld) / pvecback[pba->index_bg_rho_fld] + theta_shift; // N-body gauge correction
    }
    ...
    if (ppt->has_source_theta_dr == _TRUE_)
    {
      ...
      _set_source_(ppt->index_tp_theta_dr) = 3. / 4. * k * y[ppw->pv->index_pt_F0_dr + 1] / f_dr + theta_shift; // N-body gauge correction
    }
    ...
    {
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
        _set_source_(ppt->index_tp_theta_ur) = y[ppw->pv->index_pt_theta_ur] + theta_shift; // N-body gauge correction
      else
        _set_source_(ppt->index_tp_theta_ur) = ppw->rsa_theta_ur + theta_shift; // N-body gauge correction
    }
    ...
    {
      if (ppw->approx[ppw->index_ap_rsa_idr] == (int)rsa_idr_off)
        _set_source_(ppt->index_tp_theta_idr) = y[ppw->pv->index_pt_theta_idr] + theta_shift; // N-body gauge correction
      else
        _set_source_(ppt->index_tp_theta_idr) = ppw->rsa_theta_idr + theta_shift; // N-body gauge correction
    }
    ...
    if (ppt->has_source_theta_ncdm == _TRUE_)
    {
      ...
      {
        _set_source_(index_tp) = ppw->theta_ncdm[index_tp - ppt->index_tp_theta_ncdm1] + theta_shift; // N-body gauge correction
      }
    }
  }
  ...
  if (_tensors_)
  {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
    {
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off)
      {
        ...
        P = -(1. / 10. * y[ppw->pv->index_pt_delta_g] + 2. / 7. * y[ppw->pv->index_pt_shear_g] + 3. / 70. * y[ppw->pv->index_pt_delta_g + 4] - 3. / 5. * y[ppw->pv->index_pt_pol0_g] + 6. / 7. * y[ppw->pv->index_pt_pol2_g] - 3. / 70. * y[ppw->pv->index_pt_pol0_g + 4]) / sqrt(6.);
      }
      else
      {
    ...
    if ((ppt->gauge == synchronous) && (ppt->get_perturbations_in_current_gauge == _FALSE_))
    {
      ...
      if (pba->has_ncdm == _TRUE_)
      {
        for (n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++)
        {
          ...
          delta_p_over_delta_rho_ncdm[n_ncdm] = (delta_p_over_delta_rho_ncdm[n_ncdm] * delta_ncdm_syn + p_prime_over_rho_ncdm * alpha) / (delta_ncdm_syn - 3 * a * H * (1. + w_ncdm) * alpha);
        }
      }
                         ...
                         ErrorMsg error_message)
{
  ...
  if (_scalars_)
  {
    ...
    if (pba->has_idm == _TRUE_)
    {
      ...
      dy[pv->index_pt_theta_idm] =
          -a_prime_over_a * theta_idm + metric_euler + k2 * c2_idm * delta_idm; /* idm velocity */
      ...
      if (pth->has_idm_dr == _TRUE_)
      {
        ...
        {
          tca_slip_idm_dr = (pth->n_index_idm_dr - 2. / (1. + S_idm_dr)) * a_prime_over_a * (theta_idm - theta_idr) + 1. / (1. + S_idm_dr) / dmu_idm_dr * (-theta_idm * (pvecback[pba->index_bg_H_prime] * a + 2. * a_prime_over_a * a_prime_over_a) - a_prime_over_a * (.5 * k2 * delta_idr + metric_euler) - k2 * c2_idm * (theta_idm + metric_continuity) + k2 / 3. * (theta_idr + metric_continuity));

          if (pth->has_idm_g == _TRUE_)
          {
            tca_slip_idm_dr += 1. / (1. + S_idm_dr) / dmu_idm_dr * ((2 + pth->n_index_idm_g) * a_prime_over_a * S_idm_g * dmu_idm_g * (theta_idm - theta_g) - S_idm_g * dmu_idm_g * (dy[pv->index_pt_theta_idm] - k2 * delta_g / 4. - metric_euler - pvecthermo[pth->index_th_dkappa] * (theta_b - theta_g) - dmu_idm_g * (theta_idm - theta_g)));
          }
          if (pth->has_idm_b == _TRUE_)
          {
            tca_slip_idm_dr += 1. / (1. + S_idm_dr) / dmu_idm_dr * (-(a_prime_over_a * R_idm_b + dR_idm_b) * (theta_idm - theta_b) - R_idm_b * (dy[pv->index_pt_theta_idm] + a_prime_over_a * theta_b - metric_euler - k2 * delta_p_b_over_rho_b - R * pvecthermo[pth->index_th_dkappa] * (theta_g - theta_b) - S_idm_b * R_idm_b * (theta_idm - theta_b)));
          }
    ...
    if (ppw->approx[ppw->index_ap_tca] == (int)tca_off)
    {
      ...
      dy[pv->index_pt_theta_b] =
          -a_prime_over_a * theta_b + metric_euler + k2 * delta_p_b_over_rho_b + R * pvecthermo[pth->index_th_dkappa] * (theta_g - theta_b);
    ...
    else
    {
      ...
      /* perturbed recombination has an impact **/
      dy[pv->index_pt_theta_b] =
          (-a_prime_over_a * theta_b + k2 * (delta_p_b_over_rho_b + R * (delta_g / 4. - s2_squared * ppw->tca_shear_g)) + R * ppw->tca_slip) / (1. + R) + metric_euler;
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
    {
      ...
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off)
      {
        ...
        dy[pv->index_pt_theta_g] =
            k2 * (delta_g / 4. - s2_squared * y[pv->index_pt_shear_g]) + metric_euler + pvecthermo[pth->index_th_dkappa] * (theta_b - theta_g);
        ...
        dy[pv->index_pt_shear_g] =
            0.5 * (8. / 15. * (theta_g + metric_shear) - 3. / 5. * k * s_l[3] / s_l[2] * y[pv->index_pt_l3_g] - photon_scattering_rate * (2. * y[pv->index_pt_shear_g] - 4. / 5. / s_l[2] * P0));
        ...
        dy[pv->index_pt_delta_g + l] =
            k * (s_l[l] * y[pv->index_pt_delta_g + l - 1] - (1. + l) * cotKgen * y[pv->index_pt_delta_g + l]) - photon_scattering_rate * y[pv->index_pt_delta_g + l];

        /** - -----> photon polarization l=0 */

        dy[pv->index_pt_pol0_g] =
            -k * y[pv->index_pt_pol0_g + 1] - photon_scattering_rate * (y[pv->index_pt_pol0_g] - 4. * P0);

        /** - -----> photon polarization l=1 */

        dy[pv->index_pt_pol1_g] =
            k / 3. * (y[pv->index_pt_pol1_g - 1] - 2. * s_l[2] * y[pv->index_pt_pol1_g + 1]) - photon_scattering_rate * y[pv->index_pt_pol1_g];

        /** - -----> photon polarization l=2 */

        dy[pv->index_pt_pol2_g] =
            k / 5. * (2. * s_l[2] * y[pv->index_pt_pol2_g - 1] - 3. * s_l[3] * y[pv->index_pt_pol2_g + 1]) - photon_scattering_rate * (y[pv->index_pt_pol2_g] - 4. / 5. * P0);
        ...
        dy[pv->index_pt_pol0_g + l] =
            k * (s_l[l] * y[pv->index_pt_pol0_g + l - 1] - (l + 1) * cotKgen * y[pv->index_pt_pol0_g + l]) - photon_scattering_rate * y[pv->index_pt_pol0_g + l];
      }
      ...
      else
      {
        ...
        /* perturbed recombination has an impact **/
        dy[pv->index_pt_theta_g] =
            -(dy[pv->index_pt_theta_b] + a_prime_over_a * theta_b - k2 * delta_p_b_over_rho_b) / R + k2 * (0.25 * delta_g - s2_squared * ppw->tca_shear_g) + (1. + R) / R * metric_euler;
    ...
    if ((ppt->has_perturbed_recombination == _TRUE_) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off))
    {
      ...
      dy[ppw->pv->index_pt_perturbed_recombination_delta_temp] = 2. / 3. * dy[ppw->pv->index_pt_delta_b] - a * Compton_CR * pow(pba->T_cmb / a, 4) * chi / (1. + chi + fHe) * ((1. - pba->T_cmb / a / pvecthermo[pth->index_th_Tb]) * (delta_g + delta_chi * (1. + fHe) / (1. + chi + fHe)) + pba->T_cmb / a / pvecthermo[pth->index_th_Tb] * (delta_temp - 1. / 4. * delta_g));
    }
    ...
    if (pba->has_dcdm == _TRUE_)
    {
      ...
      dy[pv->index_pt_delta_dcdm] = -(y[pv->index_pt_theta_dcdm] + metric_continuity) - a * pba->Gamma_dcdm / k2 * metric_euler; /* dcdm density */
    ...
    if (pba->has_fld == _TRUE_)
    {
      ...
      if (pba->use_ppf == _FALSE_)
      {
        ...
        dy[pv->index_pt_delta_fld] =
            -(1 + w_fld) * (y[pv->index_pt_theta_fld] + metric_continuity) - 3. * (cs2 - w_fld) * a_prime_over_a * y[pv->index_pt_delta_fld] - 9. * (1 + w_fld) * (cs2 - ca2) * a_prime_over_a * a_prime_over_a * y[pv->index_pt_theta_fld] / k2;

        /** - ----> fluid velocity */

        dy[pv->index_pt_theta_fld] = /* fluid velocity */
            -(1. - 3. * cs2) * a_prime_over_a * y[pv->index_pt_theta_fld] + cs2 * k2 / (1. + w_fld) * y[pv->index_pt_delta_fld] + metric_euler;
      }
      else
      {
    ...
    if (pba->has_scf == _TRUE_)
    {
      ...
      if (pba->model_cdm == 2)                                                                              // KBL: for interacting DM, additional terms appear in the KG perturbation equation
      {
        ...
        dy[pv->index_pt_mom_scf] += -a2 * (pvecback[pba->index_bg_rho_cdm] * (2.0 * pba->cdm_c * pba->cdm_c * tanh_c_phi * (1.0 + tanh_c_phi)) * y[pv->index_pt_phi_scf] // -a^2 m'' n delta_phi
      ...
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
      {
        ...
        else
        {
          ...
          if (ppr->ur_fluid_approximation == ufa_mb)
          {
            ...
            dy[pv->index_pt_shear_ur] =
                -3. / tau * y[pv->index_pt_shear_ur] + 2. / 3. * (y[pv->index_pt_theta_ur] + metric_shear);
          }
          ...
          if (ppr->ur_fluid_approximation == ufa_hu)
          {
            ...
            dy[pv->index_pt_shear_ur] =
                -3. * a_prime_over_a * y[pv->index_pt_shear_ur] + 2. / 3. * (y[pv->index_pt_theta_ur] + metric_shear);
          }
          ...
          if (ppr->ur_fluid_approximation == ufa_CLASS)
          {
            ...
            dy[pv->index_pt_shear_ur] =
                -3. / tau * y[pv->index_pt_shear_ur] + 2. / 3. * (y[pv->index_pt_theta_ur] + metric_ufa_class);
          }
        }
      }
    }
  ...
  if (_vectors_)
  {
    ...
    if (ppt->gauge == synchronous)
    {
      ...
      dy[pv->index_pt_theta_b] = -(1 - 3. * cb2) * a_prime_over_a * y[pv->index_pt_theta_b] - pvecthermo[pth->index_th_dkappa] * (_SQRT2_ / 4. * delta_g + y[pv->index_pt_theta_b]);
    }

    else if (ppt->gauge == newtonian)
    {

      dy[pv->index_pt_theta_b] = -(1 - 3. * cb2) * a_prime_over_a * y[pv->index_pt_theta_b] - _SQRT2_ / 4. * pvecthermo[pth->index_th_dkappa] * (delta_g + 2. * _SQRT2_ * y[pv->index_pt_theta_b]) + pvecmetric[ppw->index_mt_V_prime] + (1. - 3. * cb2) * a_prime_over_a * y[pv->index_pt_V];
    }
    ...
    */
    ...
    P1 = -_SQRT6_ / 40. * (4. / (3. * k) * theta_g // F1
                           + y[pv->index_pt_delta_g + 3] + 2. * y[pv->index_pt_pol0_g] + 10. / 7. * y[pv->index_pt_pol2_g] - 4. / 7. * y[pv->index_pt_pol0_g + 4]);
    ...
    if (ppt->gauge == synchronous)
    {
      ...
      dy[pv->index_pt_delta_g] =
          -4. / 3. * theta_g - pvecthermo[pth->index_th_dkappa] * (delta_g + 2. * _SQRT2_ * y[pv->index_pt_theta_b]);

      /* photon velocity (theta_g = (3k/4)*F_1) */
      dy[pv->index_pt_theta_g] =
          k2 * (delta_g / 4. - s_l[2] * shear_g) - pvecthermo[pth->index_th_dkappa] * (theta_g + 4.0 / _SQRT6_ * P1) + 4.0 / (3.0 * _SQRT2_) * ssqrt3 * y[pv->index_pt_hv_prime];
    }
    ...
    else if (ppt->gauge == newtonian)
    {
      ...
      /* photon density (delta_g = F_0) */
      dy[pv->index_pt_delta_g] =
          -4. / 3. * theta_g - pvecthermo[pth->index_th_dkappa] * (delta_g + 2. * _SQRT2_ * y[pv->index_pt_theta_b]) - 2. * _SQRT2_ * pvecmetric[ppw->index_mt_V_prime];

      /* photon velocity (theta_g = (3k/4)*F_1) */
      dy[pv->index_pt_theta_g] =
          k2 * (delta_g / 4. - s_l[2] * shear_g) - pvecthermo[pth->index_th_dkappa] * (theta_g + 4.0 / _SQRT6_ * P1);
    }

    /* photon shear (shear_g = F_2/2) */
    dy[pv->index_pt_shear_g] =
        4. / 15. * s_l[2] * theta_g - 3. / 10. * k * s_l[3] * y[pv->index_pt_shear_g + 1] - pvecthermo[pth->index_th_dkappa] * shear_g;

    /* photon l=3 */
    dy[pv->index_pt_l3_g] =
        k / 7. * (6. * s_l[3] * shear_g - 4. * s_l[4] * y[pv->index_pt_l3_g + 1]) - pvecthermo[pth->index_th_dkappa] * y[pv->index_pt_l3_g];

    /* additional momenta in Boltzmann hierarchy (beyond l=0,1,2,3,4) */
    for (l = 4; l < pv->l_max_g; l++)
      dy[pv->index_pt_delta_g + l] =
          k / (2. * l + 1.) * (l * s_l[l] * y[pv->index_pt_delta_g + l - 1] - (l + 1.) * s_l[l + 1] * y[pv->index_pt_delta_g + l + 1]) - pvecthermo[pth->index_th_dkappa] * y[pv->index_pt_delta_g + l];

    /* l=lmax */
    l = pv->l_max_g;
    dy[pv->index_pt_delta_g + l] =
        k * (s_l[l] * y[pv->index_pt_delta_g + l - 1] - (1. + l) * cotKgen * y[pv->index_pt_delta_g + l]) - pvecthermo[pth->index_th_dkappa] * y[pv->index_pt_delta_g + l];

    /* photon polarization, l=0 (pol0_g = G_0)*/
    dy[pv->index_pt_pol0_g] =
        -k * y[pv->index_pt_pol0_g + 1] - pvecthermo[pth->index_th_dkappa] * (y[pv->index_pt_pol0_g] - _SQRT6_ * P1);

    /* additional momenta in Boltzmann hierarchy (beyond l=0,1,2,3,4) */
    for (l = 1; l < pv->l_max_pol_g; l++)
      dy[pv->index_pt_pol0_g + l] =
          k / (2. * l + 1.) * (l * s_l[l] * y[pv->index_pt_pol0_g + l - 1] - (l + 1.) * s_l[l + 1] * y[pv->index_pt_pol0_g + l + 1]) - pvecthermo[pth->index_th_dkappa] * y[pv->index_pt_pol0_g + l];

    /* l=lmax */
    l = pv->l_max_pol_g;
    dy[pv->index_pt_pol0_g + l] =
        k * (s_l[l] * y[pv->index_pt_pol0_g + l - 1] - (l + 1.) * cotKgen * y[pv->index_pt_pol0_g + l]) - pvecthermo[pth->index_th_dkappa] * y[pv->index_pt_pol0_g + l];
  ...
  if (_tensors_)
  {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off)
    {
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off)
      {
        ...
        /* (P^{(2)}) */
        P2 = -1.0 / _SQRT6_ * (1. / 10. * delta_g + 2. / 7. * shear_g + 3. / 70. * y[pv->index_pt_delta_g + 4] - 3. / 5. * y[pv->index_pt_pol0_g] + 6. / 7. * y[pv->index_pt_pol2_g] - 3. / 70. * y[pv->index_pt_pol0_g + 4]);
        ...
        */
        ...
        /* photon density (delta_g = F_0) */
        dy[pv->index_pt_delta_g] =
            -4. / 3. * theta_g - pvecthermo[pth->index_th_dkappa] * (delta_g + _SQRT6_ * P2)
            //+y[pv->index_pt_gwdot];
            + _SQRT6_ * y[pv->index_pt_gwdot]; // TBC

        /* photon velocity (theta_g = (3k/4)*F_1) */
        dy[pv->index_pt_theta_g] =
            k2 * (delta_g / 4. - s_l[2] * shear_g) - pvecthermo[pth->index_th_dkappa] * theta_g;
        ...
        /* photon polarization, l=0 (pol0_g = G_0)*/
        dy[pv->index_pt_pol0_g] =
            -k * y[pv->index_pt_pol0_g + 1] - pvecthermo[pth->index_th_dkappa] * (y[pv->index_pt_pol0_g] - _SQRT6_ * P2);
    ...
    if (ppt->evolve_tensor_ur == _TRUE_)
    {
      ...
      dy[pv->index_pt_shear_ur] = (4. / 15. * y[pv->index_pt_theta_ur] - 3. / 10. * k * s_l[3] / s_l[2] * y[pv->index_pt_shear_ur + 1]);
                                     ...
                                     ErrorMsg error_message)
{
  ...
  if (ppr->tight_coupling_approximation >= (int)second_order_CLASS)
  {
    ...
    if (ppr->tight_coupling_approximation == (int)second_order_CLASS)
    {
      F_prime_prime = (-pvecthermo[pth->index_th_dddkappa] * tau_c * tau_c /* F'' needed by second_order_CLASS only */
                       + 2. * pvecthermo[pth->index_th_ddkappa] * pvecthermo[pth->index_th_ddkappa] * tau_c * tau_c * tau_c) /
                          (1 + R) +
                      2. * dtau_c * a_prime_over_a * R / (1 + R) / (1 + R) + tau_c * ((a_primeprime_over_a - 2. * a_prime_over_a * a_prime_over_a) + 2. * a_prime_over_a * a_prime_over_a * R / (1 + R)) * R / (1 + R) / (1 + R);
    }
  }
  ...
  if (ppr->tight_coupling_approximation == (int)first_order_MB)
  {
    ...
    slip = 2. * R / (1. + R) * a_prime_over_a * (theta_b - theta_g) + F * (-a_primeprime_over_a * theta_b + k2 * (-a_prime_over_a * delta_g / 2. + cb2 * (-theta_b - metric_continuity) - 4. / 3. * (-theta_g - metric_continuity) / 4.) - a_prime_over_a * metric_euler);
  }
  ...
  if ((ppr->tight_coupling_approximation == (int)first_order_CAMB) || (ppr->tight_coupling_approximation == (int)compromise_CLASS))
  {
    ...
    slip = (dtau_c / tau_c - 2. * a_prime_over_a / (1. + R)) * (theta_b - theta_g) + F * (-a_primeprime_over_a * theta_b + k2 * (-a_prime_over_a * delta_g / 2. + cb2 * (-theta_b - metric_continuity) - 4. / 3. * (-theta_g - metric_continuity) / 4.) - a_prime_over_a * metric_euler);
  }
  ...
  if ((ppr->tight_coupling_approximation == (int)first_order_CLASS) || (ppr->tight_coupling_approximation == (int)second_order_CLASS))
  {
    ...
    slip = (dtau_c / tau_c - 2. * a_prime_over_a / (1. + R)) * (theta_b - theta_g) + F * (-a_primeprime_over_a * theta_b + k2 * (-a_prime_over_a * delta_g / 2. + pvecthermo[pth->index_th_dcb2] * delta_b + cb2 * (-theta_b - metric_continuity) - 4. / 3. * (-theta_g - metric_continuity) / 4.) - a_prime_over_a * metric_euler);
  }
  ...
  {
    slip += -F * dmu_idm_g * (k2 * delta_g / 4. + metric_euler + pvecthermo[pth->index_th_dkappa] * (theta_b - theta_g) + dmu_idm_g * (theta_idm - theta_g) - theta_idm_prime);
  }

  if (pth->has_idm_b == _TRUE_)
  {
    slip -= S_idm_b * tau_c / tau_idm_b / (1. + R) * ((a_prime_over_a - dtau_idm_b / tau_idm_b) * (theta_idm - theta_b) + theta_idm_prime - (-a_prime_over_a * theta_b + metric_euler + cb2 * k2 * delta_b + R / tau_c * (theta_g - theta_b) + S_idm_b * R_idm_b * (theta_idm - theta_b)));
  }
  ...
  if (ppr->tight_coupling_approximation == (int)second_order_CRS)
  {
    ...
    if (ppt->gauge == synchronous)
    {
      ...
      Delta = 2 * k2 * y[pv->index_pt_eta] - 2 * a_prime_over_a * pvecmetric[ppw->index_mt_h_prime] - pvecmetric[ppw->index_mt_h_prime_prime];

      /* monster expression for slip at second-order in tight-coupling */
      slip = (-2. / (1. + R) * a_prime_over_a - pvecthermo[pth->index_th_ddkappa] / pvecthermo[pth->index_th_dkappa]) * (theta_b - theta_g) + (-a_primeprime_over_a * theta_b - k2 * a_prime_over_a * (delta_g / 2. - 2. * shear_g) + k2 * (cb2 * (-theta_b - metric_continuity) - 4. / 3. * (-theta_g - metric_continuity) / 4. + shear_g_prime)) / pvecthermo[pth->index_th_dkappa] / (1. + R) - 2. * R * (3. * a_prime_over_a * a_prime_over_a * cb2 + (1. + R) * (a_primeprime_over_a - a_prime_over_a * a_prime_over_a) - 3. * a_prime_over_a * a_prime_over_a) / (1. + R) / (1. + R) / (1. + R) * (theta_b - theta_g) / pvecthermo[pth->index_th_dkappa] + (a_primeprime_over_a * a_prime_over_a * ((2. - 3. * cb2) * R - 2.) * theta_b / (1. + R) + a_prime_over_a * k2 * (1. - 3. * cb2) * theta_b / 3. / (1. + R) + a_primeprime_over_a * k2 * cb2 * delta_b / (1. + R) + k2 * k2 * (3. * cb2 - 1.) * cb2 * delta_b / 3. / (1. + R) + k2 * k2 * R * (3. * cb2 - 1.) * delta_g / 12. / (1. + R) + a_primeprime_over_a * k2 * (2. + 3. * R) * delta_g / 4. / (1. + R) + a_prime_over_a * a_prime_over_a * k2 * ((2. - 3. * cb2) * R - 1.) * delta_g / 2. / (1. + R) + a_prime_over_a * k2 * cb2 * (1. + (3. * cb2 - 2.) * R) * (-theta_b - metric_continuity) / (1. + R) + a_prime_over_a * k2 * (2. + (5. - 3. * cb2) * R) * 4. / 3. * (-theta_g - metric_continuity) / 4. / (1. + R) + a_prime_over_a * (1. - 3. * cb2) * k2 * 2. * metric_shear / 3. + k2 * k2 * (3. * cb2 - 1.) * y[pv->index_pt_eta] / 3. + 2. * a_prime_over_a * k2 * (3. * cb2 - 1.) * pvecmetric[ppw->index_mt_eta_prime] + k2 * (1. - 3. * cb2) * Delta / 6.) / pvecthermo[pth->index_th_dkappa] / pvecthermo[pth->index_th_dkappa] / (1. + R) / (1. + R) - (4. * a_primeprime_over_a * theta_b - 4. * k2 * cb2 * (-theta_b - metric_continuity) + 2. * a_prime_over_a * k2 * delta_g + k2 * 4. / 3. * (-theta_g - metric_continuity)) / 2. / (1. + R) / (1. + R) * pvecthermo[pth->index_th_ddkappa] / pvecthermo[pth->index_th_dkappa] / pvecthermo[pth->index_th_dkappa] / pvecthermo[pth->index_th_dkappa] + 4. * a_prime_over_a * R / (1. + R) / (1. + R) * pvecthermo[pth->index_th_ddkappa] / pvecthermo[pth->index_th_dkappa] / pvecthermo[pth->index_th_dkappa] * (theta_b - theta_g);
  ...
  if (ppr->tight_coupling_approximation == (int)second_order_CLASS)
  {
    ...
    if (ppt->gauge == synchronous)
    {
      ...
      theta_prime_prime = ((R - 1.) * a_prime_over_a * theta_prime - (a_primeprime_over_a - a_prime_over_a * a_prime_over_a) * theta_b + k2 * (pvecthermo[pth->index_th_dcb2] * delta_b + cb2 * (-theta_b - metric_continuity) - a_prime_over_a * R / 4. * delta_g + R / 4. * 4. / 3. * (-theta_g - metric_continuity))) / (1. + R);
      ...
      g0_prime_prime = -a_prime_over_a * theta_prime_prime - 2. * (a_primeprime_over_a - a_prime_over_a * a_prime_over_a) * theta_prime - (2. * a_prime_over_a * a_prime_over_a * a_prime_over_a - 3. * a_primeprime_over_a * a_prime_over_a) * theta_b + k2 * (pvecthermo[pth->index_th_ddcb2] * delta_b - 2. * pvecthermo[pth->index_th_dcb2] * (theta_b + 0.5 * pvecmetric[ppw->index_mt_h_prime]) + (1. / 3. - cb2) * (theta_prime + 0.5 * pvecmetric[ppw->index_mt_h_prime_prime]));

      /* slip at second order */
      slip = (1. - 2 * a_prime_over_a * F) * slip + F * k2 * s2_squared * (2. * a_prime_over_a * shear_g + shear_g_prime) - F * (F_prime_prime * g0 + 2. * F_prime * g0_prime + F * g0_prime_prime);
    ...
    ErrorMsg error_message)
{
  ...
  if (ppt->gauge == newtonian)
  {
    ...
    if (ppr->radiation_streaming_approximation == rsa_MD_with_reio)
    {
      ...
      ppw->rsa_theta_g +=
          3. / k2 * (ppw->pvecthermo[pth->index_th_ddkappa] * y[ppw->pv->index_pt_theta_b] + ppw->pvecthermo[pth->index_th_dkappa] * (-a_prime_over_a * y[ppw->pv->index_pt_theta_b] + ppw->pvecthermo[pth->index_th_cb2] * k2 * y[ppw->pv->index_pt_delta_b] + k2 * y[ppw->pv->index_pt_phi]));
    }
  ...
  if (ppt->gauge == synchronous)
  {
    ...
    else
    {
      ...
      ppw->rsa_delta_g = 4. / k2 * (a_prime_over_a * ppw->pvecmetric[ppw->index_mt_h_prime] - k2 * y[ppw->pv->index_pt_eta]);
    ...
    if (ppr->radiation_streaming_approximation == rsa_MD_with_reio)
    {
      ...
      ppw->rsa_theta_g +=
          3. / k2 * (ppw->pvecthermo[pth->index_th_ddkappa] * (y[ppw->pv->index_pt_theta_b] + 0.5 * ppw->pvecmetric[ppw->index_mt_h_prime]) + ppw->pvecthermo[pth->index_th_dkappa] * (-a_prime_over_a * y[ppw->pv->index_pt_theta_b] + ppw->pvecthermo[pth->index_th_cb2] * k2 * y[ppw->pv->index_pt_delta_b] - a_prime_over_a * ppw->pvecmetric[ppw->index_mt_h_prime] + k2 * y[ppw->pv->index_pt_eta]));
    }
    ...
    if (pba->has_ur == _TRUE_)
    {
      ...
      {
        ppw->rsa_delta_ur = 4. / k2 * (a_prime_over_a * ppw->pvecmetric[ppw->index_mt_h_prime] - k2 * y[ppw->pv->index_pt_eta]);
